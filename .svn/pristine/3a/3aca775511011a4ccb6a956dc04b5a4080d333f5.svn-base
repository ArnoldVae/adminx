<template>
	<div class="scene-config">
		...
		<div class="scene-table-main" ref="scene-table-main">
			...
			<searche-header-wrapper ref="searche-header-wrapper">
				...
				<form-item
					title="场景编号"
					type="text"
					v-model="params.vcCode"
					placeholder="输入场景编号搜索"
					:clearable="true"
				></form-item>
				...
				<div style="width: 100%;">
					<com-button
						size="large"
						type="add"
						:disabled="!canAddSceneOrg"
						@click="handleBtn('add-sce-org')"
					>添加场景组织</com-button>

					<com-button
						size="large"
						type="edit"
						:disabled="!canActionSceOrg"
						@click="handleBtn('edit-sce-org')"
					>修改场景组织</com-button>
					...
					<com-button
						...
						:disabled="!canActionSceOrg"
						@click="handleBtn('delete-sce-org')"
					>删除场景组织</com-button>

					<com-button
						size="large"
						type="add"
						:disabled="!canActionScene"
						@click="handleBtn('add-scene')"
					>添加场景</com-button>
					<com-button
						size="large"
						type="delete"
						:disabled="!canActionScene && !selectionsIds.length > 0"
						@click="handleBtn('batch-delete-scene')"
					>批量删除</com-button>
				</div>
		...
		>
			...
			<modal-footer
				slot="footer"
				@on-cancel="sceneModal.set(false)"
				@on-confirm="handleModalConfirm(sceneModal.type)"
			></modal-footer>
		</Modal>
	</div>
</template>
	...
	data() {
		return {
			...
			columns: [
				...
				{ label: '状态', prop: 'isEnableHtml', width: 100 },
			...
			this.http.getSceneData(params).then(res => {
				if (res.code == 200) {
					...
					this.tableData = res.data.lists.map(item => {
						...
						item['isEnableHtml'] =
							item['isEnable'] == 1
								? this.returnHtmlString('span', '启用', '#19be6b')
								: item['isEnable'] == 0
								? this.returnHtmlString('span', '禁用', '#ff9900')
								: ''
