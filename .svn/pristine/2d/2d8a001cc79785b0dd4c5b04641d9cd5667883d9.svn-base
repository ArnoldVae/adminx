...
export default {
	...
	methods: {
		...
		async getStationTree() {
			let result = await this.axios.getUnitTree({ iFlag: 0 })
			if (result.code == 200) {
		...
		async getStationTable() {
			...
			let result = await this.axios.getStationList(JSON.stringify(params))
			if (result.code == 200) {
			...
			if (this.isAdd) {
				...
				this.validateForm('add-form', () => {
					if (this.addFormData.vcName) {
						...
						if (
							...
							this.axios.saveStation(JSON.stringify(params)).then(res => {
								if (res.code == 200) {
									this.modalShow = false
									this.upModalShow = false
				...
				this.validateForm('up-form', () => {
					...
					if ((reg.test(this.upFormData.dMapx) && reg.test(this.upFormData.dMapy)) || (this.upFormData.dMapy == '' && this.upFormData.dMapx == '')) {
						...
						this.axios.updateStation(JSON.stringify(params)).then(res => {
							if (res.code == 200) {
								this.modalShow = false
								this.upModalShow = false
			...
			if (data == 1) {
				...
				this.$Modal.confirm({
					...
					onOk: () => {
						...
						this.axios.delStation(JSON.stringify(removeData)).then(res => {
		...
		handleLinkStationSave() {
			if (this.linkData && this.stationUnitId) {
				...
				this.$Message.warning('请选择接入的变电站')
			}
		},
		// 查询字典组
		findDic() {
