<template>
  <div class="SvgaddIndex">
    ...
    <div class="table-wrapper">
      <searche-Header-Wrapper><Button type="success"
                size="large"
                icon="md-add"
                @click="addPage()">新增</Button></searche-Header-Wrapper>
      ...
      <Table height="500"
        ...
        <template slot-scope="{ row, index }"
          ...
          <Button type="primary"
                  ...
                  @click="Editor(row, index)">编辑</Button>
					<Button type="warning" icon="ios-create-outline" style="margin-right: 10px" @click="upData(row, index)">修改</Button>
					<Button type="error" icon="md-trash" @click="remove(row, index)" style="margin-right: 5px">删除</Button>
					<Button type="success" icon="md-create" @click="Editor2(row, index)">编辑2</Button>
				</template>
			</Table>
			<!-- 新增列表 -->
			...
			<!-- 编辑弹窗2 -->
			<Modal v-model="newConfigModal" id="new-config-modal-wrap" title="编辑图纸" fullscreen footer-hide>
				<new-config-modal
					class="new-config-modal"
					v-if="newConfigModal"
					:url="subUrl"
					:id="subId"
					:pageId="subPageId"
...
import newConfigModal from './new-sub-config-modal'
export default {
	name: 'SvgaddIndex',
	data() {
		return {
			...
			modalShow: false,
			configModal: false,
			newConfigModal: false, // 新增加的编辑ht页面
			treeData: [],
			columns: [
				{
					title: '名称',
			...
			ruleValidate: {
				vcName: [{ required: true, message: '名称不能为空', trigger: 'blur' }, { pattern: /^.{0,63}$/, message: '最多输入64字', trigger: 'change' }],
	...
	methods: {
		...
		newSubChange(subFlag) {
			this.newConfigModal = subFlag
		},
		addPage() {
