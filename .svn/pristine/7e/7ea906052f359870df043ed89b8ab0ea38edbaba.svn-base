	...
	<div class="sm-type-manage">
		<div class="top-search">
			<searche-Header-Wrapper>
				<form-item title="SM名称" type="text" v-model="searchData.vcName" placeholder="输入SM名称查询" noMBottom></form-item>
			...
			<Form ref="add-form" :rules="ruleValidate" :model="formData" :label-width="120" label-position="right">
				...
				<FormItem label="模型名称" prop="devTypeId">
					<Select v-model="formData.devTypeId" filterable>
						<Option v-for="item in devList" :key="item.devTypeId" :value="item.devTypeId + ''">{{ item.vcName }}</Option>
					</Select>
				</FormItem>
				<!-- 状态 -->
				<FormItem label="状态">
...
export default {
	...
	data() {
		return {
			axios: this.$api.acConfig.smTypeManage,
			modalShow: false,
			pageFlag: false,
			modalTitle: '新增',
			isAdd: true,
