	...
	<div class="data-dict">
		<div class="form-operation">
			<searche-header-wrapper>
				<form-item :disabled="loading" title="字典组编号" type="number" v-model="searchInfo.number" placeholder="输入编号搜索"></form-item>
				<form-item :disabled="loading" title="字典组名称" type="text" v-model="searchInfo.name" placeholder="输入名称搜索" clearable></form-item>
				<form-item
					:disabled="loading"
					type="select"
					title="状态"
				...
				<Button :disabled="loading" type="info" icon="md-search" class="search" @click="handleQueryDictList">查询</Button>
				<Button :disabled="loading" type="primary" icon="md-refresh" class="reset" @click="handleResetQueryDictList">重置</Button>
				<Button :disabled="loading" type="success" icon="md-add" class="add" @click="handleOpenAddModalGroup">新增</Button>
			</searche-header-wrapper>
		</div>
		<div class="table">
			<Table @on-row-dblclick="handleExpandRow" height="670" border highlight-row :columns="columns" :loading="loading" :data="tableData">
