const ____factory = function(BMap) {
   ...
   */
	...
	InfoBox.prototype.setStyles = function(style) {
		this._opts.boxStyle = style
		baidu.dom.setStyles(this._div, this._opts.boxStyle)
	}
	...
	InfoBox.prototype.open = function(anchor) {
		...
		} else if (anchor instanceof BMap.Overlay) {
	...
	baidu.object.extend(InfoBox.prototype, {
		...
		_setContent: function(content) {
			...
			var closeHtml = (this._closeIcon = this._getCloseIcon())
			// string类型的content
		...
		_adjustPosition: function(poi) {
			...
			switch (this._opts.align) {
				...
				case INFOBOX_AT_BOTTOM:
					if (this._marker) {
						this._div.style.top =
							pixel.y + this._opts.offset.height - icon.anchor.height + icon.infoWindowAnchor.height + this._marker.getOffset().height + 'px'
					} else {
		...
		_panBox: function() {
			...
			switch (this._opts.align) {
				...
				case INFOBOX_AT_BOTTOM:
					// 下侧超出
					this._marker ? -icon.anchor.height + icon.infoWindowAnchor.height + this._marker.getOffset().height + this._opts.offset.height : 0
...
export const InfoBoxFactory = ____factory
