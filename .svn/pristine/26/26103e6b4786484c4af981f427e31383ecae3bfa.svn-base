const ____factory = function() {
	 ...
	 *
	 */
	var RectangleZoom = function(map, opts) {
		if (!map) {
			return
		}
	...
	RectangleZoom.prototype.open = function() {
		...
		var endDrawRect = function(e) {
			...
			var northEastPoint = bds.getNorthEast()
			var southEastPoint = new BMap.Point(northEastPoint.lng, southWestPoint.lat)
			var northWestPoint = new BMap.Point(southWestPoint.lng, northEastPoint.lat)
			var rect = new BMap.Polygon([southWestPoint, northWestPoint, northEastPoint, southEastPoint])
...
export const RectangleZoom = ____factory()
