const ____factory = function() {
	...
	baidu.object.extend(LuShu.prototype, {
		...
		_setInfoWin: function(pos) {
			...
			var infoWindow = me._opts.showInfoWindow
			if (index != -1) {
				clearInterval(me._intervalFlag)
				me._overlay.setHtml(me._opts.landmarkPois[index].html)
				me._overlay.setPosition(pos, me._marker.getIcon().size)
				me._pauseForView(index)
				!me._opts.defaultContent && me._opts.showInfoWindow && me.showInfoWindow()
			} else {
				me._overlay.setHtml(me._opts.defaultContent)
				!me._opts.defaultContent && me._opts.showInfoWindow && me.hideInfoWindow()
			}
			me._opts.showInfoWindow = infoWindow
		},
...
export const LuShu = ____factory()
