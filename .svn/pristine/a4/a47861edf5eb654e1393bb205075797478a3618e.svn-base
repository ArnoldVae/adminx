                    ...
                    <template slot-scope="scope">
                      <span v-if="scope.row.rowShow">
                        ...
                        <div
                          class="keyList"
                          v-if="MenuShow"
                          :style="{'top':dispostionTop+'px','left':dispostionLeft+'px'}"
                        >
                    ...
                    <template slot-scope="scope">
                      <span v-if=" scope.row.noderowShow">
                        ...
                        <div
                          class="keyList"
                          v-if="startNodeShow"
                          :style="{'top':startNodeTop+'px','left':startNodeLeft+'px'}"
                        >
                          <Menu width="auto" mode="vertical">
                            <MenuItem
                              :name="item.id"
...
export default {
	...
	methods: {
		...
		async getStationTree() {
			let result = await this.$api.svgFlowConfig.getUnitTree({ iFlag: 3 })
				...
				.then(res => {
					if (res.code == 200) {
						if (res.data) {
							...
							this.nodeListsearch = []
							res.data.forEach(item => {
								if (item.vcValueDesc != null) {
									if (!this.nodeArr.includes(item.vcValueDesc)) {
										this.nodeArr.push(item.vcValueDesc)
									}
								}
		...
		listClick(items) {
			...
			if (items.vcMemo) {
				...
				selectDatas.forEach(sles => {
					// console.log(slesforEach
					this.optionDats.push({
						value: sles.substring(1),
						id: sles.substring(0, 1)
					})
				})
				this.optionDats.forEach(its => {
					this.options.push({
						label: its.value,
						value: its.id
					})
		...
		Edit(row, index) {
			...
			if (index >= 3) {
				this.dispostionTop = -143
			} else {
				this.dispostionTop = 45
			}
		...
		//保存
		save(item, index) {
			if (this.matchDatas(this.keySearch) == this.keySearch) {
				this.MenuShow = false
				this.options.forEach(its => {
					if (this.value == its.value) {
						this.jianName = its.label
					}
				})
				if (this.keySearch == '' || this.jianName == '') {
					this.$message({
						message: '点位名称或监视值不能为空',
						type: 'warning'
					})
				} else {
					this.$api.svgFlowConfig
						...
						.then(res => {
							if (res.code == 200) {
								...
								this.dispostionData[index].vcMemo = this.jianName //监视值
								this.dispostionData[index].longSortid = this.sortName.length == 0 ? 0 : this.sortName //排序
		...
		matchDatas(val) {
			if (val) {
				let result = ''
				this.nodeListsearch.forEach(item => {
					if (item.vcName === val) {
						result = item.vcName
					}
				})
		...
		// 新增
		add() {
			// console.log(this.dispostionData.length)
			this.dispostionData.forEach((item, idx) => {
				if (item.rowShow) {
					this.dispostionData[idx].rowShow = false
				}
			})
			if (this.dispostionData.length >= 3) {
				this.dispostionTop = -143
			} else {
				this.dispostionTop = 45
			}
		...
		startAdd() {
			...
			if (this.startNoneData.length >= 3) {
				this.startNodeTop = -143
			} else {
				this.startNodeTop = 45
			}
		...
		startNodeEdit(row, index) {
			...
			this.nodeOptions.forEach(item => {
				if (this.startNodevalue == item.label) {
					this.startNodevalue = item.value
				}
			})
			if (index >= 3) {
				this.startNodeTop = -143
			...
			if (this.startNodekeySearch.length === 0) {
				this.nodeOptions = []
			}
		},
		...
		startNodesave(item, index) {
			if(this.matchDatas(this.startNodekeySearch) == this.startNodekeySearch){
					this.nodeOptions.forEach(its => {
				// console.log(its);
				if (this.startNodevalue == its.value) {
					this.startName = its.label
				}
			})
			this.MenuShow = false
			if (this.startNodekeySearch == '' || this.startName == '') {
				this.$message({
					message: '点位名称或启动值不能为空',
					type: 'warning'
				})
			} else {
				this.$api.svgFlowConfig
					.saveNodeData({
						...
						intStart: 1,
						vcMemo: this.startName,
						vcName: this.startNodekeySearch,
						vcUnitName: this.untidName,
						vcProtectName: this.protectAreaName,
						floatValue: this.startNodevalue
					})
					.then(res => {
						if (res.code == 200) {
							this.$message({
							...
							this.startNoneData[index].vcMemo = this.startName //启动值
							this.startNoneData[index].longSortid = this.startNodeSortName.length == 0 ? 0 : this.startNodeSortName //排序
