...
export function createIcon(BMap, options = {}) {
	...
	return new BMap.Icon(url, createSize(BMap, size), {
		...
		size: opts.size && createSize(BMap, opts.size),
 ...
 */
export function isPointInRect(BMap, point, bounds) {
	// 检查类型是否正确
 ...
 */
export function isPointInCircle(BMap, point, circle) {
	// 检查类型是否正确
	...
	var dis = getDistance(BMap, point, c)
	if (dis <= r) {
		return true
	} else {
		return false
 ...
 */
export function isPointOnPolyline(BMap, point, polyline) {
	// 检查类型
 ...
 */
export function isPointInPolygon(BMap, point, polygon) {
	// 检查类型
 ...
 */
export function getDistance(BMap, point1, point2) {
	// 判断类型
 ...
 */
export function getPolylineDistance(BMap, polyline) {
	// 检查类型
	if (polyline instanceof BMap.Polyline || polyline instanceof Array) {
		...
		for (var i = 0; i < pts.length - 1; i++) {
			var curPt = pts[i]
			var nextPt = pts[i + 1]
			var dis = getDistance(BMap, curPt, nextPt)
			totalDis += dis
		}
 ...
 */
export function getPolygonArea(BMap, polygon) {
	// 检查类型
	...
	for (var i = 0; i < Count; i++) {
		...
		AngleCos =
			(AHtangent * ALtangent + BHtangent * BLtangent + CHtangent * CLtangent) /
