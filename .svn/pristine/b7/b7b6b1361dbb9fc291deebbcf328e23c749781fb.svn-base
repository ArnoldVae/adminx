<template>
  <div class="linkage-config">
    <!-- 左侧树 -->
    <div class="lf-tree">
      <search-tree @on-select-change="treeClick"
                   placeholder="请输入变电站名称"
                   :data="treeList"></search-tree>
    </div>
    <!-- 右侧主体内容 -->
    <div class="rt-content">
      <div class="top-header">
        ...
        <div class="btn-box">
          <Button type="success"
                  size="large"
                  icon="md-add"
                  @click="handleConfig('add')">配置</Button>
        </div>
      </div>
      ...
      <Modal v-model="modalShow"
        ...
        <div slot="footer">
          ...
          <Button type="primary"
                  size="large"
                  @click="handleSaveLink">确认</Button>
        </div>
        <div class="modalCtn">
          <!-- 模态框顶部内容 -->
          <div class="modal-top"
               v-if="modalTab">
            <span @click="handleModalTabs(item)"
                  v-for="(item, idx) in modalData.tabList"
                  :key="idx"
                  :class="item.active == true ? 'myTabs active' : 'myTabs'">{{ item.name }}</span>
          </div>
          <div class="modal-top"
               v-if="!modalTab">
          ...
          <div class="modal-content"
            ...
            <div class="modalCtn-lf">
              <!-- 联动节点树 -->
              <div class="modalTree-lf"
                   v-loading="leftLoading">
                <div class="lf-top">联动节点</div>
                <Tree :data="linkTree"
                      @on-select-change="handleLfLinkTree"></Tree>
              </div>
              <!-- 报警节点树 -->
              <div class="modalTree-rt"
                   v-loading="rightLoading">
                <div class="lf-top">执行节点</div>
                <Tree :data="configTree"
                      @on-select-change="handleRtLinkTree"></Tree>
              </div>
            </div>
    ...
    <Modal v-model="clientModal"
           title="客户端屏幕"
           footer-hide
           :mask-closable="false">
      <ul class="client-box">
        <li v-for="(item, index) in clients"
            :key="index"
            @click="handleClient(item)">{{ item }}</li>
      </ul>
    </Modal>

    <Modal v-model="tvModal"
           title="解码服务"
           :width="450"
           footer-hide
           :mask-closable="false">
      <div>选择解码服务</div>
      <Select v-model="tvSelect"
              style="width:400px">
        <Option v-for="item in tvCityList"
                :value="item.value"
                :key="item.value">{{ item.label }}</Option>
      </Select>
      <Tabs value="name1">
        <TabPane label="一画面"
                 name="name1">
          <ul class="screen1">
            <li v-for="(item, index) in tvNum"
                :key="index"
                @click="tvClick(1, item)">{{ '输出' + item }}</li>
          </ul>
        </TabPane>
        <TabPane label="四画面"
                 name="name2">
          <ul class="screen4">
            <li v-for="(item, index) in tvNum"
                :key="index">
              <span v-for="(ite, idx) in [1, 2, 3, 4]"
                    :key="idx"
                    @click="tvClick(4, item, ite)">{{ ite }}</span>
            </li>
          </ul>
        </TabPane>
        <TabPane label="九画面"
          ...
          <ul class="screen9">
            <li v-for="(item, index) in tvNum"
                :key="index">
              <span v-for="(ite, idx) in [1, 2, 3, 4, 5, 6, 7, 8, 9]"
                    :key="idx"
                    @click="tvClick(9, item, ite)">{{ ite }}</span>
            </li>
          </ul>
        </TabPane>
      </Tabs>
...
export default {
	...
	methods: {
		...
		// 弹窗内Tabs点击
		handleModalTabs(tar) {
			if (this.modalData.nodeList.data.length == 0) {
				this.type = tar.value
				...
				let index = this.modalData.tabList.findIndex(item => item.value == this.modalData.selectTab)
				this.$Message.warning(`您已选择${this.modalData.tabList[index].name}的数据,请先提交数据或者清空数据后切换！`);
			}
		},
		// 弹窗内删除
		delDevList(params) {
