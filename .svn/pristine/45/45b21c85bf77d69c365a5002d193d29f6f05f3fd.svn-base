...
export default {
			...
			sceneFormItems: [
				{ label: '站端', prop: 'station', disabled: true },
				{
					...
					rules: [
						...
						{ pattern: /^.{0,31}$/, message: '最多输入32字', trigger: 'change' }
					]
				},
				{
					label: '场景名称',
					...
					rules: [
						{ required: true, message: '该项为必填项', trigger: 'change' },
						{ pattern: /^.{0,127}$/, message: '最多输入128字', trigger: 'change' }
					]
				},
				{
					label: '场景类型',
				...
				{ label: '备注', prop: 'vcMemo', rules: [{ pattern: /^.{0,31}$/, message: '最多输入32字', trigger: 'change' }] }
			],
	...
	methods: {
		...
		editSceneInitInfo() {
			...
			this.sceneForm.isEnable = JSON.parse(JSON.stringify(this.editScene.isEnable)) + ''
			this.sceneForm.vcMemo = JSON.parse(JSON.stringify(this.editScene.memo))
			// console.log(this.editScene)
		...
		disposeParams() {
			let { vcName, vcCode, iType, isEnable, vcMemo } = this.sceneForm
			let obj = {
				unitId: this.station.id,
