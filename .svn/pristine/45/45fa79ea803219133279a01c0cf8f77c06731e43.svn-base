  ...
  <div class="sm-type-manage">
    <div class="top-search">
      <searche-Header-Wrapper>
        ...
        <form-item title="筛选"
                   type="select"
                   v-model="searchData.selectNum"
                   :options="selectList">
        </form-item>
        <Button type="info"
                size="large"
                icon="md-search"
                @click="searchConfig">查询</Button>
        <Button type="primary"
                size="large"
                icon="md-refresh"
    ...
    <Modal v-model="modalShow"
      ...
      <Form ref="add-form"
        ...
        <FormItem label="模型名称"
          ...
          <Select v-model="formData.devTypeId"
                  filterable>
            <Option v-for="item in devList"
                    :key="item.devTypeId"
                    :value="item.devTypeId + ''">{{ item.vcName }}</Option>
          </Select>
        </FormItem>
        <!-- 状态 -->
        <FormItem label="状态">
      ...
      <div slot="footer">
        ...
        <Button type="primary"
                size="large"
                @click="handleSaveSmType">确认</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
	...
	data() {
		return {
			...
			searchData: { vcName: '', selectNum: 1 },
			selectList: [{ value: 2, label: '启用' }, { value: 0, label: '禁用' }],
			devList: [],
			tableColumns: [
				{
					title: 'SM名称',
		...
		getTable(type) {
			...
			if (!this.pageFlag) {
				...
				params.acSmtypeEntity = {
					vcName: '',
					iFlag: this.searchData.selectNum == 2 ? '' : this.searchData.selectNum
				}
			} else {
				params.page = {
					pageNum: type == 1 ? 1 : this.page,
