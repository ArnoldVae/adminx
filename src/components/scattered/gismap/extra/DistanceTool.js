const ____factory = function() {
	...
	var DistanceTool = function(map, opts) {
		...
		this._opts = baidu.extend(
			baidu.extend(this._opts || {}, {
				 ...
				 * @type {String}
				 */
				cursor: `${global.mapconfig.home}/images/ruler.cur`,
	...
	DistanceTool.prototype.open = function() {
		...
		var distClick = function(e) {
			...
			var path = new BMap.Polyline([poi, poi], {
			...
			if (me._points.length > 1) {
				// 非起点的节点，显示当前的距离
				me._setSegDistance(me._points[me._points.length - 2], me._points[me._points.length - 1])
				var meters = me._getTotalDistance()
				disText = me._formatDisStr(meters)
			} else {
				disText = '起点'
			}
			var disLabel = new BMap.Label(disText, {
		...
		this.bindFunc = [
			{
				elem: OperationMask.getDom(this._map),
				type: 'click',
				func: distClick
			},
			{
	...
	DistanceTool.prototype._clearCurData = function() {
		...
		for (var j = 0, len = this._paths.length; j < len; j++) {
			this._paths[j].remove()
		}
		for (var k = 0, len_k = this._dots.length; k < len_k; k++) {
			this._dots[k].remove()
		}
		this._initData()
	}
	...
	DistanceTool.prototype._addSecPoint = function(pt) {
		var BMap = window.BMap
		var ico = this._opts.secIcon
			? this._opts.secIcon
			: new BMap.Icon(`${global.mapconfig.home}/images/mapctrls.png`, new BMap.Size(11, 11), {
	...
	DistanceTool.prototype.setSecIcon = function(icon) {
		var BMap = window.BMap
		this._opts.secIcon = icon instanceof BMap.Icon ? icon : this._opts.secIcon
		for (var i = 0, _marker; (_marker = this._dots[i]); i++) {
			_marker.setIcon(this._opts.secIcon)
		}
	}
	...
	DistanceTool.prototype._processLastOp = function() {
		...
		pt.disLabel = new BMap.Label('', {
			offset: new BMap.Size(-15, -40),
			enableMassClear: me._enableMassClear
		})
		pt.disLabel.setStyles({
			color: '#333',
			borderColor: '#ff0103'
		})
		...
		// 添加关闭按钮
		var bico = this._opts.closeIcon
			? this._opts.closeIcon
			: new BMap.Icon(`${global.mapconfig.home}/images/mapctrls.gif`, new BMap.Size(12, 12), {
		...
		disObj.closeBtn.addEventListener('click', function(e) {
			...
			for (var j = 0, len = disObj.paths.length; j < len; j++) {
				disObj.paths[j].remove()
				disObj.paths[j] = null
			}
			for (var k = 0, len_k = disObj.dots.length; k < len_k; k++) {
				disObj.dots[k].remove()
				disObj.dots[k] = null
			}
	...
	DistanceTool.prototype._formatTitle = function(type, text, distance, label) {
		...
		if (type == 1) {
			...
			dis = this._convertUnit(distance, u)
			if (dis > this._units[u].incon) {
				dis = dis / this._units[u].incon
				unit = this._units[u].u2
				dis = dis.toFixed(1)
			...
			// 结束时的总距离展示
			dis = this._convertUnit(this._getTotalDistance(), u)
			if (dis > this._units[u].incon) {
				dis = dis / this._units[u].incon
				unit = this._units[u].u2
				dis = dis.toFixed(1)
...
export const DistanceTool = ____factory()
