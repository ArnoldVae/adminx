	...
	methods: {
		...
		filterTreeNode(treeData, target) {
			...
			result = treeData.filter(item => {
				if (target.value == '') {
					return true
				} else if (/^[\u4e00-\u9fa5]+$/gi.test(target.value) && item.title.includes(target.value) && target.value != '') {
					this.checked && (item.checked = true)
					this.selected && (item.selected = true)
					item.children = this.filterTreeNode(item.children, target)
					return true
				} else if (
					// item.pinyin.includes(target.value.toLowerCase()) 全拼搜索
					escape(target.value).indexOf('%u') < 0 &&
					item.py.includes(target.value.toLowerCase()) &&
					target.value != ''
				) {
					this.checked && (item.checked = true)
					this.selected && (item.selected = true)
