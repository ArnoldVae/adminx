		...
		<div class="right-content">
			<div class="tool-bar">
				<searche-Header-Wrapper>
					<div class="btn-box">
						<Button type="success" size="large" icon="md-cloud-upload" @click="handleUploadBtn(0)">导入维保计划</Button>
					</div>
				</searche-Header-Wrapper>
			</div>
			<!-- 表格区 -->
		...
		<Modal v-model="modalShow" title="文件上传" :mask-closable="false">
			<div class="upload-content">
				<Steps class="steps" :current="step" title="文件上传步骤">
					<Step title="步骤一" content="择上传文件 (支持.xls格式的文件)"></Step>
...
export default {
	...
	methods: {
		...
		upload() {
			// 点击上传
			if (this.isFile) {
				if (this.file.name.substring(this.file.name.length - 4) == 'xlsx' || this.file.name.substring(this.file.name.length - 4) == 'XLSX') {
