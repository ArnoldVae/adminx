<template>
  <div class="PSvgcontainer">
    ...
    <Input class="Input"
           v-model="InputValue"
           @on-enter="inputEnter(InputValue)"
    ...
    <Button class="btn"
            type="primary"
            @click="getTreeView">一键保存</Button>
    <Button class="btn returnBtn"
            @click="returnList"
            type="warning">返回列表</Button>
    <div id="svgCon"
         class="svgCon"></div>
    <!-- 对话框 -->
    <Modal v-model="isDialog"
           title="设备选择"
           :mask-closable="false"
           :styles="{ top: '10px' }"
           width="80%"
           footer-hide>
      <!-- 选择设备 -->
      <device-modeling-modal v-if="isDialog"
                             @callBack="callBack" />
    </Modal>
  </div>
</template>
...
export default {
	...
	methods: {
		...
		isDrag(vcSourceId, isFalse) {
			...
			let material = this.palette_config.material.items
			// 设备数据查找
			...
			// 素材数据查找
			let materialIndex = material.findIndex(item => item.devId == vcSourceId)
			if (materialIndex != -1) {
				material[materialIndex].draggable = isFalse
				// console.log(scene[sceneIndex]);
			}
		},
		...
		inputEnter(val) {
			...
			arr.forEach(item => {
				...
				if (item.isType == '设备') {
					...
					child.setIcon('assets/libs/ht/storage/assets/svg/material.png')
					child.a('nType', 5)
					child.setTag(item.devId)
				}
				child.a('item', item)
